...
The refinement takes raw depth as input and never touches backbone internals, so the real test is whether it helps a model it was never tuned against. Depth Anything V2 (Small) and MiDaS DPT-Hybrid — different architectures (DINOv2 vs. ViT-Hybrid), different training recipes — both run through the identical pipeline on 6 scenes from Middlebury Stereo 2014, against real laser/stereo ground truth:
